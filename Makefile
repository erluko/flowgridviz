...
$(pcap_dest_file): $(npm_package_config_pcap_file)
	tshark -r $< -Tfields -E occurrence=f -Eseparator=, -e ip.src -e ip.dst -e udp.srcport -e udp.dstport -e tcp.srcport -e tcp.dstport $(NUM_RECORDS_PRE) $(npm_package_config_num_records) 'tcp or udp' |gzip -c > $@
