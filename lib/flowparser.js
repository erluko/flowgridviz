...
function BadData(msg,data,lineno){
  this.message=msg;
  this.data=data;
  this.lineno=lineno;
}

BadData.prototype.toString = function(){
  return this.message +" @" +this.lineno+": "+JSON.stringify(this.data)
}
...
function ipStrToInt(s){
  ...
  let octets = s.split('.');
  if(octets.some(o=>isNaN(o) || o>255 || o<0)){
    throw new BadData("Invalid IPv4 address",[octets,s])
  }
...
let parseLine =  function (fp,line){
  ...
  */
  ...
  if(row.splice(0,4).some(x=>isNaN(x))){
    throw new BadData("Non-numeric value found in first five elements",[line,row]);
  }
  let label = row[5];
    ...
    rl.on('line', function(line){
      if(fp.max==0 || ridx<fp.max){
        ...
        if(ridx == 0 && (line[0]>'9' || line[0]<'0')){
          ...
          try{
            let r = parseLine(fp,line);
            fp.rows[ridx++] = r;
          } catch (e){
            console.log("BAD LINE"+line);
            //annotate with the bad line number
