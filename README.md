...
file into a 53x53 grid. Each grid point represents the intersection of
src and dest port or IP. The darkness of each point is based on the
number of such pairs present in the packet capture.

The hashing function used to make the initial reduction down to the
coordinate space is intentionally reversible so that the list of actually
